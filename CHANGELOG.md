...
## v0.5.7
- **新增「默认模式」设置**（帮助面板 ▸ 显示设置）：可选 **阅读模式（默认）/ 双栏模式 / 编辑模式**，存本机（`mdr-default-mode`），即时生效。
- **启动与新开文档不再沿用上次模式**：此前 `setMode` 会把上次使用的模式写入 `mdr-mode`，且新建文档会继承当前文档的模式，导致一旦切过双栏，之后所有文档都以双栏打开。现在统一以上面设置的「默认模式」为准。
- 仍可在工具栏随时切换模式（仅影响当前标签页/本次会话）。

## v0.5.6
- **新增：阅读模式就地编辑（源码级，100% 保真）**。阅读模式下**双击任意一段**（段落 / 标题 / 引用 / 代码块 / 整个列表或表格），该段就地变成编辑框，内容是**这段的原始 Markdown 源码**，`⌘/Ctrl+Enter` 保存、`Esc` 取消、点击别处自动保存。
  - 实现上是把该块的源码行区间（`data-source-line` ~ `data-source-line-end`）替换回原文，**不做 HTML→Markdown 反向转换**，因此表格、KaTeX 公式、代码块、Mermaid 图表与你的原始语法（`-`/`*`、缩进、换行）完全不被改写。
  - 渲染时新增写入块级结束行 `data-source-line-end`（此前只有起始行）；双击时自动向上取到最外层块（编辑整个列表/表格而非单个 `li`/`td`）。
  - 保存后自动重新渲染并触发自动保存，同时同步当前标签页的源码与脏状态。
  - 双栏模式双击行为不变（仍是定位到左侧编辑器对应行）。

## v0.5.5
- **优化：全屏 / 大窗口下正文只占中间一条、两侧大片空白**。根因是正文宽度固定为 `--content-width: 46rem`（约 736px），不随视口变化。
  - 默认值改为**随视口自适应放宽** `clamp(46rem, 62vw, 88rem)`：小窗口保持舒适行宽，全屏或大窗口时正文自动变宽（如 1512 宽约 937px、2560 宽约 1408px），充分利用屏幕。
  - 新增「正文宽度」设置（帮助面板 ▸ 显示设置）：紧凑 `46rem` / 标准（自适应，默认）/ 宽松 `clamp(52rem, 82vw, 112rem)` / 铺满窗口 `100%`，选择即时生效并保存在本机（`mdr-content-width`）。

## v0.5.4
- **修复：「文件 ▾」下拉菜单透明度过高、看不清**。根因为菜单沿用工具栏的半透明 `var(--bar-bg)`（亮色 0.8 / 纸张 0.85 / 暗色 0.85），浮于正文上方时透出底层文字。新增不透明 `--menu-bg` 卡片色（亮色 `#ffffff` / 纸张 `#fbf2e0` / 暗色 `#26292e`），菜单改用该实色并加深阴影，三套主题均清晰可读。
